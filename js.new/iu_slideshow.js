	...
	var methods = {
		init : function( options ) {		
			...
			return rval = this.each(function(){	
				...
				var slideshowButtonsWidth = 2+(1.5*numSlides);
				$('#slideshow-buttons').css({'width':slideshowButtonsWidth+'em'});
		...
		fadeIn : function () {
			...
			$($('img', $this).get(cur)).parent().fadeOut(state.fadeOutInterval);
			$($('img', $this).get(next)).parent().fadeIn(state.fadeInInterval);
