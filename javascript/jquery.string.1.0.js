...
jQuery.extend({
	__stringPrototype: {
		...
		/**
		 * Converts a string separated by spaces into an array.
		 * For instance, 'foo bar' would be converted to a = ['foo','Bar'].
		 * @param {String} s string to be evaluated
		 * @return array
		 */
		arrayize: function(pattern, s) {
			var a = this.s(s).split(pattern), i;
			return a;
		},
		/**
